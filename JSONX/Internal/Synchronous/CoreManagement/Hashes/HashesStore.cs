...
namespace Internal.Synchronous.CoreManagement.Hashes
{
    public partial class HashesClass
    {
        ...
        internal HashesClass(ClassEngine engine)
        {
            _Engine = engine;
        }
        ...
        public bool Rename_HashesClass(string oldKey, string newKey)
        {
            var data = _Engine.Load();
        ...
        public bool RenameField_HashesClass(string key, string oldField, string newField)
        {
            var data = _Engine.Load();
            if (!(data.ContainsKey(key) && IsHashes(data[key]))) return false;
            var dataDic = ConvertToDictionary(data[key]);
